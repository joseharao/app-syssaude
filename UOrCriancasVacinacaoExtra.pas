unit UOrCriancasVacinacaoExtra;
...
type
  TfrmOrRVacinaExtra = class(TfrmOrPadrao)
    GroupBox7: TGroupBox;
...
var
  frmOrRVacinaExtra: TfrmOrRVacinaExtra;

implementation

{$R *.dfm}

uses UDados, UFuncoes, URVacinacao, URVacinacaoExtra;

procedure TfrmOrRVacinaExtra.btImprimirClick(Sender: TObject);
begin
  inherited;
  if application.MessageBox('Deseja Imprimir Listagem Agora?', 'Confirmação', MB_YesNo + MB_IconQuestion) = id_Yes then begin
    sql := 'SELECT * FROM LISTA_VACINACAO L WHERE L.COD_VACINA > 0';
    ...
    application.CreateForm(TfrmRVacinacaoExtra, frmRVacinacaoExtra);
    with frmRVacinacaoExtra do begin
      cdsTab.Close;
      sqlTab.Close;
      sqlTab.SQL.Clear;
      sqlTab.SQL.Add(sql);
...
procedure TfrmOrRVacinaExtra.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfrmOrRVacinaExtra.FormShow(Sender: TObject);
begin
  inherited;
